...
const os = require('os');
const debug = require('debug')('compass:app:package-manager');
...
/**
 * The name of the internal packages directory.
 */
const INTERNAL_PATH_NAME = 'packages';

/**
 * The fully qualified name to the internal packages.
 */
const INTERNAL_PACKAGES_PATH = path.join(path.resolve(path.dirname(__dirname)), INTERNAL_PATH_NAME);

/**
 * Exported container reside here.
 */
const PACKAGE_PATHS = [ INTERNAL_PACKAGES_PATH, CPM_PACKAGES_PATH ];
...
class PackageManager {
  ...
  _isReadingComplete() {
    return (this.expectedPackageCount === this.packages.length) &&
      (this.directoriesScannedCount === PACKAGE_PATHS.length);
  }
  ...
  _read() {
    if (this.packages.length === 0) {
      _.each(PACKAGE_PATHS, (packagesPath) => {
