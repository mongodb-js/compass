...
describe('Streams', function(){
  ...
  it.skip('(@todo lucas: not sure why this passes on it\'s own but fails in all tests) ' +
      'should allow streaming top #slow', function(done){
  ...
  describe('Collection.createWriteStream()', function(){
    var dest;
    ...
    it('should support a batchSize option', function(done){
      ...
      bulk = dest.createWriteStream({batchSize: 50})
      ...
      datasets(50, {n: '{{chance.d10()}}'}).pipe(bulk);
    });

    it('should insert all docs if #docs < batchSize', function(done){
      var complete = false;
      datasets(10, {n: '{{chance.d10()}}'})
        .pipe(dest.createWriteStream({batchSize: 100}).on('end', function(){
