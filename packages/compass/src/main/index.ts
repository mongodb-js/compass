...
// executed at runtime.
//
// Dev-mode note: in `npm start` builds, webpack-dev-server injects an HMR
// client into this bundle that logs `[HMR] Waiting for update signal from
// WDS...` to stdout before any of our code runs. That would corrupt the
// MCP wire when stdio is the transport. AI clients route through
// `packages/compass-mcp-server/scripts/dev-stdio-bridge.js` in dev (see
// `getBridgeInvocation` in compass-mcp-server-manager); the wrapper sits
// between the AI client and this bundle and filters non-JSON lines out
// of stdout. Production builds have no HMR and bypass the wrapper.
