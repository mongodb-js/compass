...
const browserWindowsThatHaveDisconnectedAtLeastOnce = new Set<BrowserWindow['id']>();

export function shouldWindowAutoConnect({ id }: Pick<BrowserWindow, 'id'>): boolean {
  ...
  return autoConnectWindow === id && !browserWindowsThatHaveDisconnectedAtLeastOnce.has(id);
}
