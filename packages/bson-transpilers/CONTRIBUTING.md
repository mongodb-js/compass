...
It is a subclass of the "empty" visitor class generated in `lib/antlr`.
...
There is a lot of repeated code between input visitors, so that has been moved into
`codegeneration/CodeGenerationVisitor.js`. Each language-specific visitor will define `get` methods
that wrap around the tree so that the generic visitor does not need to know or
