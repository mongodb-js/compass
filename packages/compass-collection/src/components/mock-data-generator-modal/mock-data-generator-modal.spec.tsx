  ...
  describe('on the script result step', () => {
    ...
    it('shows userConnectionString in the mongosh command when available', async () => {
      ...
      expect(screen.getByText(mockUserConnectionString, { exact: false })).to
        .exist;
      expect(screen.queryByText('mongodb://localhost:27017/', { exact: false }))
        .to.not.exist;
    });

    it('shows the active connection string when there is no Atlas metadata', async () => {
      const nonAtlasConnectionInfo: ConnectionInfo = {
        id: 'test-desktop-connection',
        connectionOptions: { connectionString: 'mongodb://localhost:27017' },
      };
      ...
      expect(screen.getByText('mongodb://localhost:27017/', { exact: false }))
        .to.exist;
    });
  });
