...
async function saveUpdateQueryAndRunIt(
  browser: CompassBrowser,
  telemetry: Telemetry,
  {
    name,
    onBeforeOpeningFavorites,
  }: {
    name: string;
    onBeforeOpeningFavorites?: () => Promise<void>;
  }
) {
  ...
  // Fill in a name
  await browser.setValueVisible(Selectors.BulkUpdateFavouriteNameInput, name);
  ...
  await onBeforeOpeningFavorites?.();
...
describe('Bulk Update', () => {
  ...
  it('can save an update query as a favourite and return to it immediately after saving', async function () {
    skipForWeb(this, "can't use saved queries in compass-web yet");
    await saveUpdateQueryAndRunIt(browser, telemetry, {
      name: 'My Update Query - Immediate Open',
    });
  });

  it('can save an update query as a favourite and return to it in a new tab', async function () {
    skipForWeb(this, "can't use saved queries in compass-web yet");
    await saveUpdateQueryAndRunIt(browser, telemetry, {
      name: 'My Update Query - New Tab',
      onBeforeOpeningFavorites: async () => {
