...
describe('MongoDB Assistant (with mocked backend)', function () {
  ...
  before(async function () {
    try {
      ...
      sendMessage = async (
        ...
        const existingMessages = await browser.getDisplayedMessages();
        ...
        switch (expectedResult) {
          case 'success':
            await browser.waitUntil(async () => {
              const newMessages = await browser.getDisplayedMessages();
              return (
  ...
  describe('before opt-in', function () {
    ...
    it('does not send the message if the user declines the opt-in', async function () {
      ...
      expect(await browser.getDisplayedMessages()).to.deep.equal([]);
    ...
    describe('entry points', function () {
      describe('explain plan entry point', function () {
        ...
        it('does not send request when user cancels confirmation', async function () {
          ...
          const finalMessages = await browser.getDisplayedMessages();
