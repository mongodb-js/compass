...
describe('#handleUpdateConnectionOptions', function () {
  ...
  ['password', 'socks'].forEach((currentTab) => {
    describe(`validate ${currentTab} form`, function () {
      it('hosts field', function () {
        ...
        expect(response.errors[0].fieldName).to.equal(
          MARKABLE_FORM_FIELD_NAMES.IS_SSH
        );
      ...
      it('username field', function () {
        ...
        expect(
          ...
          'username is required with password'
  ...
  describe('validate identity form', function () {
    ...
    it('username field', function () {
      ...
      expect(
        response.errors[0].errors,
        'username is required with file'
      ).to.have.key('username');
    });
    ...
    it('passphrase field', function () {
      ...
      expect(
        Object.keys(response.errors[0].errors),
        'username is required with passphrase'
      ).to.contain('username');
      expect(
        Object.keys(response.errors[0].errors),
        'identityKeyFile is required with passphrase'
      ).to.contain('identityKeyFile');
    });
  });
});
