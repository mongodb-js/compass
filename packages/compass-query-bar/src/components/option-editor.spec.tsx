...
describe('OptionEditor', function () {
  ...
  describe('handles unsafe integers', function () {
    it('shows warning icon if query.filter contains unsafe integers', async function () {
      ...
      render(
        <OptionEditor
          ...
          value={`{ a: ${Number.MAX_SAFE_INTEGER + 1} }`}
          favoriteQueries={[]}
          recentQueries={[]}
          onApplyQuery={applyFromHistory}
        ></OptionEditor>
      ...
      // Callback should be called with fixed value
      await waitFor(() => expect(onChangeSpy.called).to.be.true);
      expect(onChangeSpy.firstCall.args[0]).to.equal(
        `{ a: Int64("${Number.MAX_SAFE_INTEGER + 1}") }`
      );

      // Now the button should not be visible anymore
      await waitFor(
        () =>
          expect(screen.getByTestId('option-editor-warning-empty-space')).to
            .exist
      );
    });

    const useCasesWithNoUnsafeIntegerHandling = [
      {
        name: 'project',
        value: `{ a: ${Number.MAX_SAFE_INTEGER + 1} }`,
      },
      ...
      {
        name: 'collation',
        value: `{ locale: "en", strength: ${Number.MAX_SAFE_INTEGER + 1} }`,
      },
      {
        name: 'hint',
        value: `{ a: ${Number.MAX_SAFE_INTEGER + 1} }`,
    ...
    for (const useCase of useCasesWithNoUnsafeIntegerHandling) {
      it(`does not show warning icon if query.${useCase.name} contains unsafe integers`, async function () {
        const onChangeSpy = sinon.spy();
        render(
          <OptionEditor
            optionName={useCase.name}
            namespace="test.test"
            insertEmptyDocOnFocus
            onChange={onChangeSpy}
            value={useCase.value}
        ...
        // Warning button should not be visible
        await waitFor(
          () =>
            expect(
        ...
        await waitFor(
          () =>
            expect(screen.queryByTestId('option-editor-warning-empty-space')).to
              .not.exist
        );
      });
    }
