...
Also include the query that the text question would generate in the response with a "query" key.
Do NOT wrap the response with any code block markdowns (\`\`\`json), follow the example format.
In the "query" field use the potential sub-fields "filter", "project", "limit", "sort", "collation", and "skip".
...
export function createSuggestionsUserPrompt({
  ...
  prompt.push(
    'Give 3 possible questions someone might ask of this data. Answer using a JSON array with "query" and "text" keys.'
  );
  prompt.push(
