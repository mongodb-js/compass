    ...
    context('when the type is csv', () => {
      context('when the objects are in order', () => {
        ...
        context('when the objects are not in order', () => {
          const object1 = { f1: 1, f2: 2 };
          const object2 = { f2: 4, f1: 3 };
          const trans = new DocumentTransform('csv');

          before((done) => {
            trans._transform(object1, null, done);
          });

          it('returns the string in the same order as the header', (done) => {
            trans._transform(object2, null, (error, data) => {
              expect(error).to.equal(null);
              expect(data).to.equal('3,4\n');
              done();
            });
          });
        });
