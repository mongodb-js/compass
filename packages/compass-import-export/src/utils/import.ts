...
type AnalyzedCSVFieldType = {
  ...
  // For objects only
  children?: AnalyzedCSVFields

  // For arrays only
  length?: number
};

type AnalyzedCSVField = Record<string, AnalyzedCSVFieldType>;
...
type AnalyzedCSVFields = Record<string, AnalyzedCSVField>;

type AnalyzeResult = {
  type: 'json'|'csv',
  total: number,
  delimiter?: string, // CSV only
  fields?: AnalyzedCSVFields, // CSV only
};
...
export function analyze({
  ...
  // 3. stream the whole file, counting the docs. For CSV also build up the
...
export function importJSON({
  ...
  console.log(dataService, ns, filename, abortSignal, progressCallback, stopOnErrors);
...
type ImportCSVOptions = {
  ...
  analyzedFields: AnalyzedCSVFields,
...
export function importCSV({
  ...
  // TODO: Just silencing TypeScript for now
  console.log(dataService, ns, filename, abortSignal, progressCallback, delimiter, ignoreEmptyStrings, stopOnErrors, analyzedFields, fields);
  return Promise.resolve([]);
}