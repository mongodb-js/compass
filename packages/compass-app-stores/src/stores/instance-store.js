...
store.onActivated = (appRegistry) => {
  ...
  /**
   * @param {object} data
   * @param {string} data.from The old namespace.
   * @param {string} data.to The new namespace.
   */
  async function collectionRenamed({ from, to }) {
    try {
      ...
      const metadata = await store.fetchCollectionMetadata(from);
      appRegistry.emit('refresh-collection-tabs', {
        metadata,
        newNamespace: to,
      });
      const { database } = toNS(from);
      store.refreshNamespace({
        ns: to,
        database,
      });
    } catch (e) {
      debug(`collection-renamed threw exception ${e.message}`);
    }
  }
  appRegistry.on('collection-renamed', collectionRenamed);
