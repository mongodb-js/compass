 ...
 */
export default new (class SharedExamples {
  /**
  ...
  itAddsTheEmbeddedDocumentElementToTheRootDocument() {
    ...
    it('sets the new embedded element value', function () {
      expect(this.doc.elements.at(0).elements.at(0).value).to.equal(
        'home@example.com'
      );
    });
  ...
  itAddsTheEmbeddedArrayElementToTheRootDocument() {
    ...
    it('sets the new embedded element document key', function () {
      expect(
        this.doc.elements.at(0).elements.at(0).elements.at(0).key
      ).to.equal('home');
    });

    it('sets the new embedded element document value', function () {
      expect(
        this.doc.elements.at(0).elements.at(0).elements.at(0).value
      ).to.equal('home@example.com');
    });

    it('flags the new element as added', function () {
      expect(
        this.doc.elements.at(0).elements.at(0).elements.at(0).isAdded()
      ).to.equal(true);
    });
  }
})();
