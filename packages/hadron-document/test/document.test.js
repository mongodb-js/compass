  ...
  describe('#getOriginalKeysAndValuesForSpecifiedKeys', () => {
    context('when an element is removed', function() {
      ...
      it('includes the element in the object', function() {
        expect(doc.getOriginalKeysAndValuesForSpecifiedKeys({ name: 1 })).to.deep.equal({ name: 'test' });
      });
    });
  ...
  describe('#getOriginalKeysAndValuesForFieldsThatWereUpdated', () => {
    context('when an element is removed', function() {
      ...
      it('includes the key in the object', function() {
        expect(doc.getOriginalKeysAndValuesForFieldsThatWereUpdated()).to.deep.equal({
          name: 'test'
        });
      });
    });
