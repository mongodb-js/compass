...
describe('Element', function () {
  ...
  describe('#toShellSyntax', function () {
    it('serializes element value', function () {
      const element = new Element('test', {
        a: 1,
        b: { foo: 2 },
        null_val: null,
      });
      expect(element.toShellSyntax('current', { indent: 0 })).to.equal(
        "{a:NumberInt('1'),b:{foo:NumberInt('2')},null_val:null}"
      );
    });

    it('serializes current and original values', function () {
      const element = new Element('test', {
        a: 1,
        b: 1.5,
        c: Long.fromNumber(2),
      ...
      expect(element.toShellSyntax('original', { indent: 0 })).to.equal(
        "{a:NumberInt('1'),b:Double('1.5'),c:NumberLong('2')}"
      );
    });

    it('allows specifying indent', function () {
      const element = new Element('test', { a: 1 });
      expect(element.toShellSyntax('current', { indent: '>' })).to.equal(
        "{\n>a: NumberInt('1')\n}"
      );
    });
  });
