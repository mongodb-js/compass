...
function realTimeLegend() {
  ...
  let bubbleWidth = 8;
  let onToggle = function (/* d, i, active */) {
    /* no-op */
  };
  ...
  function component(selection) {
    selection.each(function (data) {
      ...
      const legendDiv = legendGroup
        .selectAll(`div.${prefix}-item`)
        .data(data)
        .enter()
        .append('div')
      ...
      legendDiv
        ...
        .on('click', function (d, i) {
          ...
          rect.transition().duration(100).style('fill-opacity', newOpacity);
          onToggle(d, i, active);
        });

      // Add text for legend
      const textSection = legendDiv
        .append('div')
        .attr('class', `${prefix}-text`);
      textSection.append('p').attr('class', `${prefix}-linename`).text(label);

      textSection
        .append('p')
        .attr('data-testid', function (d) {
          return `performance-${label(d)}`;
        })
