...
const NetworkStore = Reflux.createStore({
  ...
  network: function (error, doc, isPaused) {
    if (!error && doc && 'localTime' in doc && 'network' in doc) {
      ...
      } else if (!isPaused && !this.isPaused && !this.starting) {
      ...
      for (let q = 0; q < this.data.dataSets.length; q++) {
        ...
        this.data.dataSets[q].count = this.bytesPerSec[key].slice(
          startPause,
          this.endPause
        );
        this.data.dataSets[q].current = count;
      ...
      this.data.secondScale.count = this.connectionCount.slice(
        startPause,
        this.endPause
      );
      this.secondCurrentMaxs.push(max(this.data.secondScale.count));
      this.data.secondScale.currentMax =
        this.secondCurrentMaxs[this.endPause - 1];
