...
describe('inValueRange [Util]', function () {
  ...
  describe('ranges with $exists', function () {
    beforeEach(function () {
      query = { $exists: true };
    });
  ...
  describe('ranges with $eq', function () {
    beforeEach(function () {
      query = { $eq: 5 };
    });
  ...
  describe('non-numeric types', function () {
    it('should work for dates', function () {
      ...
      expect(inValueRange(query, { value: new Date('2012-01-01') })).to.equal(
        'yes'
      );
      expect(inValueRange(query, { value: new Date('2015-01-01') })).to.equal(
        'no'
      );
    });
    ...
    it('should work for objectids', function () {
      query = {
        $gte: new ObjectId('578cfb38d5021e616087f53f'),
        $lte: new ObjectId('578cfb42d5021e616087f541'),
      };

      expect(
        inValueRange(query, {
          value: new ObjectId('578cfb3ad5021e616087f540'),
        })
      ).to.equal('yes');
      expect(
        inValueRange(query, {
          value: new ObjectId('578cfb6fd5021e616087f542'),
        })
      ).to.equal('no');
    });
    ...
    it('should work for $numberDecimal', function () {
      query = {
        $gte: Decimal128.fromString('1.5'),
        $lte: Decimal128.fromString('2.5'),
      };

      expect(
        inValueRange(query, { value: Decimal128.fromString('1.8') })
      ).to.equal('yes');
      expect(
        inValueRange(query, { value: Decimal128.fromString('4.4') })
      ).to.equal('no');
    });
    ...
    it('should work for $numberDecimal with a dx of 0', function () {
      ...
      expect(
        inValueRange(query, { value: Decimal128.fromString('1.8'), dx: 0 })
      ).to.equal('yes');
      expect(
        inValueRange(query, { value: Decimal128.fromString('4.4'), dx: 0 })
      ).to.equal('no');
    });

    it('should work for $numberDecimal with a single equality query', function () {
      query = Decimal128.fromString('1.5');

      expect(
        inValueRange(query, { value: Decimal128.fromString('1.5') })
      ).to.equal('yes');
      expect(
        inValueRange(query, { value: Decimal128.fromString('1.6') })
      ).to.equal('no');
    });
    ...
    it('should work for $numberDecimal with a single equality query and dx of 0', function () {
      ...
      expect(
        inValueRange(query, { value: Decimal128.fromString('1.5'), dx: 0 })
      ).to.equal('yes');
      expect(
        inValueRange(query, { value: Decimal128.fromString('1.6'), dx: 0 })
      ).to.equal('no');
    });
  });
