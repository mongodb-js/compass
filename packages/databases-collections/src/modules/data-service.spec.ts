  ...
  describe('#reducer', function() {
    ...
    context('when the action is data service connected', function() {
      it('returns the new state', function() {
        expect(reducer(undefined, dataServiceConnected('err' as any, 'ds' as any))).to.deep.equal({
          error: 'err',
          dataService: 'ds',
          configuredKMSProviders: [],
          currentTopologyType: 'Unknown'
    ...
    context('when the action is data service updated', function() {
      it('returns the new state', function() {
        ...
        const state1 = reducer(undefined, dataServiceConnected('err' as any, ds as DataService));
        ...
        expect(reducer(state2, dataServiceUpdated({} as DataService))).to.deep.equal({
          error: 'err',
          dataService: ds,
          configuredKMSProviders: [],
          currentTopologyType: 'ReplicaSetWithPrimary'
