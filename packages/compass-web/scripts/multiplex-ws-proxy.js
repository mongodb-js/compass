 ...
 */
function createMultiplexWebSocketProxy(port = 1338, logger = console) {
  ...
  wsServer.on('connection', (ws) => {
    ...
    logger.log(
      '[multiplex] new ws connection (total %s)',
      wsServer.clients.size
    );

    ws.on('error', (err) => {
      logger.log('[multiplex] ws error: %s', err.message);
    });

    ws.on('close', (code, reason) => {
      logger.log(
        '[multiplex] ws closed, code=%d, reason=%s, destroying %d stream(s)',
        code,
        reason,
        streams.size
      );
    ...
    ws.on('message', (rawData) => {
      ...
      if (!streams.has(sp)) {
        ...
        logger.log(
          '[multiplex.socket.open] tls stream sp=%d -> %s:%d',
          sp,
          da,
          dp
        );

        // All the localhost addresses will use net instead of tls.
        ...
        socket.on(connectEvent, () => {
          logger.log(
            '[multiplex.socket.%s] stream sp=%d connected to %s:%d',
            connectEvent,
            sp,
            da,
            dp
        ...
        socket.on('data', (chunk) => {
          logger.log(
            '[multiplex.socket.data] stream sp=%d sending %d bytes',
            sp,
            chunk.length
          );
          safeSend(
        ...
        socket.on('close', (hadError) => {
          logger.log(
            '[multiplex.socket.close] stream sp=%d closed, hadError=%s',
            sp,
            hadError
          );
          streams.delete(sp);
        ...
        socket.on('error', (err) => {
          logger.log(
            '[multiplex.socket.error] stream sp=%d error: %s',
            sp,
            err.message
          );
          streams.delete(sp);
...
module.exports = { createMultiplexWebSocketProxy };
