...
const CollectionItem: React.FunctionComponent<
  ...
  >
> = observer(({ item: coll, ...props }) => {
  // mobx passes object
  const name = coll.name;
  // redux only collection namespace
  // const name = coll;
  const { type } = useListedCollection(name) ?? {};
  const { data: info, status: infoStatus } = useCollectionInfo(name) ?? {};
  const { data: stats, status: statsStatus } = useCollectionStats(name) ?? {};
