...
} from '../../modules/gen-ai/suggestions-reducer';
  ...
  return (
    <li className={suggestionStyles} aria-label={text}>
      <button
        ...
        // onClick={() => onClick(EJSON.stringify(pipeline))}
        onClick={() => onClick(toJSString(pipeline) || '')}
        title={text}
      >
...
function PipelineSuggestions({
  ...
  onClickCancel: () => void;
  onClickApply: (pipeline: string) => void;
  isFetching?: boolean;
  didSucceed: boolean;
  errorMessage?: string;
      ...
      >
        ...
        {errorMessage && (
          <>
            ...
            <ErrorSummary errors={errorMessage} />
          </>
        )}
      </div>
    </InteractivePopover>
...
const ConnectedPipelineSuggestions = connect(
  (state: RootState) => ({
...
export { ConnectedPipelineSuggestions as PipelineSuggestions };
