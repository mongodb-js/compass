...
describe('StageCollaper [Component]', function () {
  context('when the stage is expanded', function () {
    ...
    beforeEach(function () {
      component = mount(<StageCollapser isExpanded index={1} onChange={spy} />);
    });
    ...
    it('renders the collapse button', function () {
      expect(component.find('Icon').first()).to.have.prop(
        'glyph',
        'ChevronDown'
      );
    });
  ...
  context('when the stage is collapsed', function () {
    ...
    beforeEach(function () {
      component = mount(
        <StageCollapser isExpanded={false} index={1} onChange={spy} />
      );
    });
