...
import { OUT_STAGE_PREVIEW_TEXT, MERGE_STAGE_PREVIEW_TEXT } from '../utils/stage';
...
const renderMergeStage = (
  ...
  render(
    <Provider store={configureStore({
      sourcePipeline: [{$match: {_id: 1}}, {$limit: 10}, {$merge: 'docs'}]
    })}>
      <MergeStage
        isLoading={false}
        isFinishedPersistingDocuments={false}
...
describe('OutputStagePreview', function () {
    ...
    context('when documents have been persisted', function () {
      context('renders the out stage preview', function () {
        it('with namespace', function () {
          renderOutStage({ isFinishedPersistingDocuments: true, destinationNamespace: 'test.out' });
          expect(screen.getByText(/Documents persisted to collection: test.out/i)).to.exist;
        });

        it('without namespace', function () {
          renderOutStage({ isFinishedPersistingDocuments: true });
          expect(screen.getByText(/Documents persisted to specified collection/i)).to.exist;
        });
      });
  ...
  describe('Merge Stage', function () {
    context('renders loader when stage is loading', function() {
      it('renders loader with namespace', function () {
        renderMergeStage({isLoading: true, destinationNamespace: 'test.docs'});
    ...
    context('when documents have been persisted', function () {
      context('renders the merge stage preview', function () {
        it('with namespace', function () {
          renderMergeStage({ isFinishedPersistingDocuments: true, destinationNamespace: 'test.docs' });
          expect(screen.getByText(/Documents persisted to collection: test.docs/i)).to.exist;
        });
