  ...
  describe('#generateStage + #generateStageAsString', () => {
    ...
    context('when the stage has an embedded document', () => {
      ...
      it('returns the stage string', () => {
        expect(generateStageAsString(stage)).to.deep.equal('{$addFields: {totalHomework: {$sum: \'$homework\'},totalQuiz: {$sum: \'$quiz\'}}}');
      });
    });
    ...
    context('when the stage has multiple types', () => {
     ...
     output: {
      ...
      it('returns the stage string', () => {
        expect(generateStageAsString(stage)).to.deep.equal('{$bucket: {groupBy: \'$price\',boundaries: [0,200,400],\'default\': \'Other\',output: {count: { $sum: 1},titles: { $push: \'$title\'}}}}');
      });
    });
    ...
    context('when the stage contains functions', () => {
      ...
      it('returns the stage string', () => {
        ...
        expect(stage.isValid).to.equal(true);
        expect(generated).to.deep.equal('{$addFields: {isFound: {$function: { body: \'function(name) {\\n return hex_md5(name) == "15b0a220baa16331e8d80e15367677ad"\\n }\', args: [ \'$name\' ], lang: \'js\'}},message: {$function: { body: \'function(name, scores) {\\n let total = Array.sum(scores);\\n return `Hello ${name}. Your total score is ${total}.`\\n }\', args: [ \'$name\', \'$scores\' ], lang: \'js\'}}}}');
      });
    });
    ...
    context('when the stage has BSON types', () => {
      ...
      it('returns the stage string', () => {
        expect(generateStageAsString(stage)).to.deep.equal('{$match: {code: Code(\'some code\'),oid: ObjectId(\'5a7382114ec1f67ae445f778\'),bin: BinData(1, \'YWFramFkZmphZGZsZGtzamZhZGY=\'),dbref: DBRef(\'coll\', \'1\'),nl: NumberLong(3),nd: NumberDecimal(\'5.00000001\'),ni: 5,minkey: MinKey(),maxkey: MaxKey(),isodate: ISODate(\'1999-01-01T00:00:00.000Z\'),regexp: RegExp(\'\\/^[a-z0-9_-]{3,16}$\\/\'),ts: Timestamp(-321469502, 367)}}');
      });
    });
  });
});
