 ...
 * https://github.com/mongodb/node-mongodb-native/blob/6ee945e7a51df925d4b6a4004c851995f832ce78/lib/core/uri_parser.js
 * with the exception of removing the parsing and resolution of srv records.
 *
 * It is used in the connection model to parse a connection string to an object,
 * this file was removed since mongodb@4.0 and we are removing the srv resolution
 * so it doesn't get called continuously each time a user changes the connection
 * string.
 */
...
const debug = require('debug')('mongodb-connection-model:uri-parser-36');
  ...
  if (authMechanism != null) {
    ...
    if (authMechanism === 'MONGODB-X509') {
      if (parsed.auth && parsed.auth.password != null) {
        throw new Error(`Password not allowed for mechanism \`${authMechanism}\``);
      }
...
function parseQueryString(query, options) {
  ...
  for (const key in parsedQueryString) {
    ...
    if (value === '' || value == null) {
      throw new Error('Incomplete key value pair for option');
    }
  ...
  if (result.wtimeout && result.wtimeoutms) {
    ...
    debug('Unsupported option `wtimeout` specified');
  }
...
function checkTLSOptions(queryString) {
  ...
  if (tlsValue != null && sslValue != null) {
    if (tlsValue !== sslValue) {
      throw new Error('All values of `tls` and `ssl` must be the same.');
    }
  }
}
    ...
    if (Array.isArray(queryString[optionName])) {
      ...
      queryString[optionName].forEach(tlsValue => {
        if (tlsValue !== firstValue) {
          throw new Error(`All values of ${optionName} must be the same.`);
        }
      });
    }
  }
...
function parseConnectionString(uri, options, callback) {
  ...
  try {
    ...
    return callback(new Error('URI malformed, cannot be parsed'));
  }
  ...
  if (authorityParts[0] == null || authorityParts[0] === '') {
    return callback(new Error('No username provided in authority section'));
  }
  ...
  if (authorityParts.length > 1) {
    ...
    if (authParts[0] === '') {
      return callback(new Error('Invalid empty username provided'));
    }
  ...
  if (directConnection && hosts.length !== 1) {
    ...
    return callback(new Error('directConnection option requires exactly one host'));
  }
