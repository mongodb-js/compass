...
export class RemoteSigningClient implements SigningClient {
  ...
  private async signRemoteFile(file: string) {
    /**
     * Passing env variables as an option to ssh.exec() doesn't work as ssh
     * config (`sshd_config.AllowEnv`) does not allow to pass env variables.
    ...
    const cmds = [
      ...
      `export docker_artifactory_username=${process.env.DOCKER_ARTIFACTORY_USERNAME}`,
      `export docker_artifactory_password=${process.env.DOCKER_ARTIFACTORY_PASSWORD}`,
      `./garasign.sh ${file}`,
    ];
