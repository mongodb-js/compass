...
import {
  getMQLForNaturalLanguageText,
  parseMQLFromAIText,
} from '../util/natural-language-processing';
...
export const MAX_DOCUMENTS_IN_NLP_PREVIEW = 4;
...
function reducer(state: NLPQueryState, action: NLPQueryAction): NLPQueryState {
  return {
    ...state,
    ...action.update,
  };
...
export function useNLPQuery({
  ...
  const onRunQuery = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        ...
        update: {
          resultErrorMessage:
            (e as Error)?.message ||
            `An error occurred when running the generated MongoDB aggregation ${
              e as string
            }`,
          resultState: 'error',
        },
    ...
    try {
      documents = await dataService
        .aggregate(namespace, aggregationPipeline)
        .limit(MAX_DOCUMENTS_IN_NLP_PREVIEW)
        .toArray();
    } catch (e) {
      if (!isMounted) {
        return;
      ...
      dispatch({
        ...
        update: {
          translateErrorMessage:
            (e as Error)?.message ||
            `Unable to translate natural language to mql, error: ${
              e as string
            }`,
          translateState: 'error',
        },
