...
describe('DataService', function () {
  ...
  describe('#shardedCollectionDetail', function () {
    it('returns the collection details', function (done) {
      service.shardedCollectionDetail(
        'data-service.test',
        function (err, detail) {
          assert.equal(null, err);
  ...
  describe('#createIndex', function () {
    after(function (done) {
      const testCollection = service.client.database.collection('test');
  ...
  describe('#sample', function () {
    it('passes arguments to native client', function () {
      const sampleSpy = sinon.spy(() => 'expectedResult');
      const dataService = new DataService(helper.connection);
      dataService.client = { sample: sampleSpy } as any;
      const res = dataService.sample('a.b', { size: 1 }, { batchSize: 10 });
  ...
  describe('#startSession', function () {
    it('passes arguments to native client', function () {
      ...
      const dataService = new DataService(helper.connection);
      dataService.client = { startSession: startSessionSpy } as any;
      const res = dataService.startSession();
      expect(res).to.equal('expectedResult');
  ...
  describe('#killSession', function () {
    it('passes arguments to native client', function () {
      ...
      const dataService = new DataService(helper.connection);
      dataService.client = { killSession: killSessionSpy } as any;
      const res = dataService.killSession({ id: 123 } as any);
      expect(res).to.equal('expectedResult');
      expect(killSessionSpy).to.have.been.calledOnceWith({ id: 123 });
    });
  });
