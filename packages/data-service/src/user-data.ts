...
const { mkdir, readFile, writeFile, readdir, unlink } = fs.promises;
...
interface NonThrowingParseFunction<T> {
...
type ParseFunction<T> = ThrowingParseFunction<T> | NonThrowingParseFunction<T>;
...
export class UserData<T = unknown> {
  ...
  private _conformParseFunctionToThrowOnError<T>(
    ...
    return (content: string) => {
      ...
      if (object === undefined) {
        throw new Error(
          (parse as NonThrowingParseFunction<T>).message ??
            'unknown parse error'
        );
      }
