...
describe('instance-detail-helper-mocked', function () {
  let makeMockDB: (errOrCallback: any, res?: any) => Db;
  let makeMockClient: (err: any, res: any) => MongoClient;
  ...
  before(function () {
     ...
     *
     * @param  {Error|null} errOrCallback    if the call should return an error, specify
     ...
     *                             driver
     */
    makeMockDB = function (errOrCallback: any, res?: any) {
      const db: any = {};
      let times = 0;
      db.admin = function () {
        return {
          // all other commands return the global err/res results
          command: function (command: any, options: any, callback: any) {
            return typeof errOrCallback === 'function'
              ? errOrCallback(times++, command, options, callback)
              : callback(errOrCallback, res);
          },
          databaseName: 'admin',
          // listCollections is a separate function on the admin object
          listCollections: function () {
            return {
              toArray: function (callback: any) {
                return callback(errOrCallback, res);
              },
            };
          },
        };
    ...
    makeMockClient = function (err, res) {
      const client: any = {};
      client.db = function (databaseName: string) {
        return {
          // all other commands return the global err/res results
          command: function (command: any, options: any, callback: any) {
  ...
  describe('getBuildInfo', function () {
    it('should pass on any error that buildInfo returns', function (done) {
      ...
      getBuildInfo(results, function (err, res) {
        ...
        assert.equal((err as any).command, 'buildInfo');
    ...
    it('should save the queryEngine field as null if it does not exist', function (done) {
      ...
      getBuildInfo(results, function (err, res) {
        assert.equal(err, null);
        assert.deepEqual((res as any).queryEngine, null);
        done();
      });
    });
  ...
  describe('getHostInfo', function () {
    ...
    it('should pass on other errors from the hostInfo command', function (done) {
      ...
      getHostInfo(results, function (err, res) {
        assert.ok(err);
        assert.equal((err as any).command, 'hostInfo');
  ...
  describe('listDatabases', function () {
    ...
    it('should ignore auth errors gracefully', function (done) {
      // instead of the real db handle, pass in the mocked one
      results.db = makeMockDB(function (
        times: any,
        command: any,
        options: any,
        callback: any
    ...
    it('should pass the read preference explicity when it is set', function (done) {
      ...
      results.db.readPreference = ReadPreference.SECONDARY;
    ...
    it('should pass on other errors from the listDatabases command', function (done) {
      ...
      listDatabases(results, function (err, res) {
        assert.ok(err);
        assert.equal((err as any).command, 'listDatabases');
        assert.deepEqual(res, null);
        done();
      });
    });
  ...
  describe('getDatabaseCollections', function () {
    ...
    it('should pass on other errors from the listCollections command', function (done) {
      ...
      getDatabaseCollections(results.db.admin(), function (err, res) {
        assert.ok(err);
        assert.equal((err as any).command, 'listCollections');
        assert.deepEqual(res, null);
        done();
      });
    });
