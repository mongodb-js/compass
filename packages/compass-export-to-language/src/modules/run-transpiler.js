...
export const runTranspiler = (input) => {
  return (dispatch, getState) => {
    ...
    const useBuilders = state.builders && !(outputLang === 'java' && state.mode === 'Pipeline');
    ...
    try {
      ...
      if (state.driver) {
        ...
        output = compiler.shell[outputLang].compileWithDriver(toCompile, useBuilders);
      } else {
        const toCompile = state.mode === 'Query' ? input.filter : input.aggregation;
        output = compiler.shell[outputLang].compile(toCompile, useBuilders, false);
      }
