...
} from '../components/connection-status-toasts';
...
/**
 * Only contains state that is required for the connection flow but can't be
 * accessed from connection storage, connections manager, or connections
 * repository
 */
type State = {
...
// All connection store actions require a connection info
type Action = { connectionInfo: ConnectionInfo } & (
  | {
    ...
    switch (action.type) {
      ...
      case 'connection-attempt-errored':
        return {
          ...
          // failed while trying to read the autoconnect info. For that case we
          // prepopulate the editing state so that the form can map the error
          // correctly
      ...
      case 'duplicate-connection':
        // When autoduplicating we just save a copy right away without opening a
        // editing modal
    ...
    async (
      ...
      try {
        ...
        if (!isAutoconnectAttempt) {
          ...
          // right now it is required that we save connection before starting the
          // connection process so that it can show up in the Compass UI, this
          // should not be happening, but we're preserving it for now to avoid
