...
var debug = require('debug')('scout-ui:test:object-tree');
...
describe('JSValueCollection', function () {
  ...
  it('should parse/serialize mixed primitive values', function () {
    var coll = new JSValueCollection([1, 'foo', false, null], {parse: true});
...
describe('JSPrimitiveValue', function () {
  ...
  it('should parse/serialize primitive values', function () {
    var val = new JSPrimitiveValue(1, {parse: true});
    assert.equal(val.serialize(), 1);
    val = new JSPrimitiveValue('foo', {parse: true});
    assert.equal(val.serialize(), 'foo');
    val = new JSPrimitiveValue(false, {parse: true});
    assert.equal(val.serialize(), false);
    val = new JSPrimitiveValue(null, {parse: true});
...
describe('JSArray', function () {
  ...
  it('should parse/serialize primitive values', function () {
    var arr = new JSArray([1, 'foo', false, null], {parse: true});
    assert.deepEqual(arr.serialize(), [1, 'foo', false, null]);
  });

  it('should parse/serialize object values', function () {
    var arr = new JSArray([{foo: 1}, {bar: 1, baz: 1}], {parse: true});
    assert.deepEqual(arr.serialize(), [{foo: 1}, {bar: 1, baz: 1}]);
  });

  it('should parse/serialize really complex arrays', function () {
    var complex = [{foo: 1, bar: [1, 2, {baz: 1}, true]}, {bar: 1, baz: {foo: null}}];
    var arr = new JSArray(complex, {parse: true});
...
describe('JSMember', function () {
  ...
  it('should parse/serialize primitive values', function () {
    var mbr = new JSMember({foo: 1}, {parse: true});
    assert.equal(mbr.value.className, 'JSPrimitiveValue');
    assert.deepEqual(mbr.serialize(), {foo: 1});

    mbr = new JSMember({foo: false}, {parse: true});
    assert.equal(mbr.value.className, 'JSPrimitiveValue');
    assert.deepEqual(mbr.serialize(), {foo: false});
  });
});
...
describe('JSMemberCollection', function () {
  ...
  it('should parse/serialize primitive values', function () {
    var coll = new JSMemberCollection([{foo: 1}, {bar: false}, {baz: null}], {parse: true});
    assert.equal(coll.at(0).className, 'JSMember');
    assert.equal(coll.length, 3);
    assert.deepEqual(coll.serialize(), [{foo: 1}, {bar: false}, {baz: null}]);
  });
});
...
describe('JSObject', function () {
  ...
  it('should parse a flat object correctly', function() {
    var obj = new JSObject({foo: 1, bar: 2}, {parse: true});
    assert.deepEqual(obj.serialize(), {foo: 1, bar: 2});
  });

  it('should parse a nested object correctly', function () {
    var nested = {foo: {bar: 1}};
    var obj = new JSObject(nested, {parse: true});
  ...
  it('should parse an object with an array correctly', function () {
    var withArray = {foo: [{foo: 1}, null, 35]};
    var obj = new JSObject(withArray, {parse: true});
  ...
  it('should parse nested arrays correctly', function () {
    var nestedArrays = {foo: [1, 2, 3, ["i", "am", "nested"]]};
    var obj = new JSObject(nestedArrays, {parse: true});
