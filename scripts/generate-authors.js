...
function getAuthorsGitLog(packagePath) {
    return execSync(`git log --reverse --format='%aN <%aE>' --use-mailmap -- ${packagePath}`, { cwd: packageRootPath }).toString().trim().split('\n');
}
...
fs.writeFileSync(path.resolve(packageRootPath, 'AUTHORS'), renderAuthorsFileContent(getAuthorsOrderedByFirstCommit('.')));
